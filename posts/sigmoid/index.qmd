---
title: "The Crucial Role of the Sigmoid Function its Derivative in Machine Learning"
...
$\sigma\left(x\right) = \frac{1}{1+e^{-x}} . \tag{1}$


This can also be expressed as:

$\sigma\left(x\right) = (1+e^{-x})^{-1} . \tag{2}$

 which we know differentiates, according to the chain rule, to:

 $-(1+e^{-x})^{-2} \times e^{-x} \times-1. \tag{3}$ 
...
$\frac{e^{-x}}{\left(1 + e ^{-x}\right)^{2}}.\tag{5}$

then we can rearrange by adding and subtracting 1 which in essence does nothing but add a zero:

$\frac{e^{-x}}{\left(1 + e ^{-x}\right)^{2}}+ 1 -1 .\tag{6}$

this easily becomes:

$\frac{1 + e^{-x}-1}{\left(1+e^{-x}\right)^{-2}}  .\tag{7}$

which is simply:

$\frac{1 + e^{-x}}{\left(1 + e ^{-x}\right)^{2}} - \frac{1}{\left(1 + e ^{-x}\right)^{2}}  .\tag{8}$

which simplifies to:

$\frac{1}{1 + e ^{-x}} - \frac{1}{\left(1 + e ^{-x}\right)^{2}}  .\tag{9}$

factorizing we get:

$\frac{1}{1 + e ^{-x}}\left(1 - \frac{1}{1 + e ^{-x}}\right). \tag{10}$
